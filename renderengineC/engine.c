...
// İki vektörün dik (çapraz) çarpımını alır (Kameranın sağını ve yukarısını bulmak için)
...
typedef struct { vec3 pos; vec3 color; float power; } Light;
...
vec3 trace_ray(Ray ray, Sphere* spheres, int num_spheres, Plane* planes, int num_planes, Light* lights, int num_lights, vec3 bg_color, int depth) {
    ...
    for (int i = 0; i < num_lights; i++) {
        ...
        if (!in_shadow) {
            ...
            vec3 diffuse = v_scale(v_mul(obj_color, lights[i].color), diff *lights[i].power);
...
void render_c(unsigned char* pixels, int width, int height,
              vec3 camera_pos, vec3 camera_dir, vec3 bg_color, // camera_dir eklendi
    ...
    // Kameranın Bakış Uzayını Hesapla (Forward, Right, Up vektörleri)
    vec3 W = v_norm(camera_dir); // İleriye bakış yönü
    vec3 world_up = (vec3){0.0f, 1.0f, 0.0f}; // Dünyanın mutlak yukarısı
    
    // Yön vektörü doğrudan yukarı veya aşağı bakıyorsa oluşabilecek sıfıra bölünme hatalarını (Gimbal Lock) önle
    if (fabs(W.y) > 0.999f) {
        world_up = (vec3){0.0f, 0.0f, 1.0f};
    }
    
    vec3 U = v_norm(v_cross(world_up, W)); // Kameranın Sağı
    vec3 V = v_cross(W, U); // Kameranın Yukarısı

    #pragma omp parallel for // İşlemci çekirdeklerini böler (OpenMP)
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            float u = (2.0f * (x + 0.5f) / width - 1.0f) * aspect_ratio;
            float v = 1.0f - 2.0f * (y + 0.5f) / height;

            // Işını kameranın yeni bakış açısına göre fırlat
            vec3 ray_dir = v_add(W, v_add(v_scale(U, u), v_scale(V, v)));
            Ray ray = {camera_pos, v_norm(ray_dir)};
